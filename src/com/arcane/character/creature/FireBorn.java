...
public class FireBorn extends Creature {
    ...
    public FireBorn() {
        super(Element.FIRE, CreatureAcronym.FIRE_BORN);
    ...
    protected String getNextRoom(GameBoard gameBoard) {
        ...
        return this.element.name() + "-" + Constants.FIRE_BORN_ROOM_DIRECTIONAL_MAP.get(currentPosition)[clockwise ? 1 : 0];
    }

    @Override
    protected String getInitialRoom() {
        // the keys of FIRE_BORN_ROOM_DIRECTIONAL_MAP contains a set of outer rooms
        // with the help of the keys list, we pick a random pair for row and column as the initial position
        String currentPosition = RandomHelper.getRandomElementFromList(
